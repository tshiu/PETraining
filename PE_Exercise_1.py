...
#Would like to get input as to what record we are looking to query
var {record_type} = prompt("What record type would you like to query?")

print record_type #print the prompt so that we can verify that the record type is valid (e.g. if they wrote in teacher ID instead of teahcer)
...
do {
	...
	#counting the length of the json response objects, assuming that if there are 4 teachers shared, there will be four responses so just counting those data blobs
	count = count + response.data.length
	#looking to maybe print here to see what might be happening in case there is a mistake
	print count #would like to see how this final number is tallied to avoid looping through the data indefinitely
...
#hwould like to continue to count until there is no pagination link at the end
